...
-- Add foreign key only if bills table exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bills') THEN
        ALTER TABLE ai_bill_summaries 
        ADD CONSTRAINT fk_ai_summaries_bill_id 
        FOREIGN KEY (bill_id) REFERENCES bills(bill_id) ON DELETE CASCADE;
    END IF;
END $$;

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_ai_bill_summaries_bill_id ON ai_bill_summaries(bill_id);
CREATE INDEX IF NOT EXISTS idx_ai_bill_summaries_generated_at ON ai_bill_summaries(generated_at);
